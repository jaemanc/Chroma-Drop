...
public static class UiTheme
{
    ...
    /// <summary>네 갈래 반짝이. 연쇄 팝업 둘레로 튀는 별 — 외부 에셋 없이 코드로 굽는다.</summary>
    public static Sprite SparkSprite()
    {
        ...
        for (int y = 0; y < S; y++)
            for (int x = 0; x < S; x++)
            {
                ...
                // 가로 갈래 + 세로 갈래 — 가운데로 갈수록 두꺼워진다
                float arm = Mathf.Max(0f, 1f - (ay * 6f + ax * 0.7f)) + Mathf.Max(0f, 1f - (ax * 6f + ay * 0.7f));
                float a = Mathf.Clamp01(arm) * Mathf.Clamp01(1f - Mathf.Sqrt(u * u + v * v));
                px[y * S + x] = new Color(1f, 1f, 1f, a);
            }
        tex.SetPixels(px);
        tex.Apply();
        spark = Sprite.Create(tex, new Rect(0, 0, S, S), new Vector2(0.5f, 0.5f), 100f);
        return spark;
    }
